...
for idx in range(len(fire_files)):
    ...
    for tidx in range(len(time_d01)):
        ...
        #     contours = plt.contourf(to_np(lons), to_np(lats), to_np(ht_850[tidx] - ht_850_2[tidx]), levels=levels, cmap=get_cmap("RdBu_r"), transform=cart_proj, linestyles='solid', alpha=0.7)
        ...
        #     plt.grid(True)
        #     plt.title("WRF 500mb Geopotential Height Difference With Creek Fire (Fire - No Fire) "+str(time_d01[tidx])[:19])
        ...
        levels = np.linspace(0, 0.0009, 50)
        qc = to_np(ht_850[tidx])
        qc[qc == 0] = np.nan
        plt.contourf(to_np(lons), to_np(lats), qc, levels=levels, transform=cart_proj, alpha=0.7, cmap='Blues')
        ax.set_xticks(np.linspace(lons.min(),lons.max(),8))
        ax.set_yticks(np.linspace(lats.min(),lats.max(),6))
        ax.set_xlabel('Longitude')
        ax.set_ylabel('Latitude')
        plt.grid(True)
        plt.title("WRF 500mb Cloud Water Mixing Ratio for Creek Fire "+str(time_d01[tidx])[:19])
        cbar = plt.colorbar(ax=ax,shrink=0.95)
        cbar.set_label('Cloud Fraction (%)', rotation=270)
        fig.savefig('fig_'+str(idx)+"_"+str(tidx)+'_500'+'.png',bbox_inches='tight',dpi=150)
        plt.close(fig)
...
# ht_850_2 = interplevel(z1, p1, 500) 
# lats, lons = latlon_coords(ht_850)
# # print(lons.min())
# # print(lons.max())
# # print(lats.min())
...
# ax.set_ylabel('Latitude')
# plt.grid(True)
# plt.title("WRF 500 hPa Geopotential Height Difference With Creek Fire (Fire - No Fire) "+sfile[11:24])
# fig.savefig(sfile[0:24]+'_500'+'.png',bbox_inches='tight',dpi=150)
# plt.close(fig)
