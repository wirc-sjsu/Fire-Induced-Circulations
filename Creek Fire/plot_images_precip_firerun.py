...
for idx in range(len(fire_files)):
    ...
    rainc = nc_f.variables['RAINC'][:]
    rainnc = nc_f.variables['RAINNC'][:]
    rain=rainc[:,:,:]+rainnc[:,:,:]
    ...
    for tidx in range(len(time_d01)):
        ...
        plt.grid(True)
        plt.title("WRF Accumulated Precipitation for Creek Fire "+str(time_d01[tidx])[:19])
