...
sdir = sorted(glob.glob('/adata/jhaw/creek_fire_runs_final/d03/wrfout_d03_2020-09-*'))
sdir2 = sorted(glob.glob('/adata/jhaw/creek_fire_runs_final/d03_nofire/wrfout_d03_2020-09-*'))
...
for f in range(len(sdir)):
    ...
    ht_surf = interplevel(z, pres, 600).data
    ...
    ht_surf1 = interplevel(z1, pres2, 600).data
...
plt.title('Min 600 hPa Geopotential Height Difference Around Active Fire (GRNHFX > 0) \nOver Time for the Creek Fire (Fire vs. No Fire)')
plt.xlabel('Time (UTC)')
