...
fire_files = sorted(glob.glob('/home/015911532/thomas_fire/d01/wrfout_d01_2017-12-0[4567]*'))
no_fire_files = sorted(glob.glob('/home/015911532/thomas_fire/d01_nofire/wrfout_d01_2017-12-0[4567]*'))
fire_line_files = sorted(glob.glob('/home/015911532/thomas_fire/d02/wrfout_d02_2017-12-0[4567]*'))
...
for idx in range(len(fire_files)):
    ...
    for tidx in range(len(time_d01)):
        ...
        plt.grid(True)
        plt.title("WRF 600 hPa Geopotential Height Difference and Wind Field \nWith Thomas Fire (Fire - No Fire) "+str(time_d01[tidx])[:19])
        #plt.clabel(contours, inline=1, fontsize=10, fmt='%1.3f',colors='black')
        fig.savefig('fig_'+str(idx)+"_"+str(tidx)+'_700'+'.png',bbox_inches='tight',dpi=150)
        plt.close(fig)
